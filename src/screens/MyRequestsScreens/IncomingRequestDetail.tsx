...
import {useGetTopLevelIncommingResponse} from './hooks/useGetTopLevelIncommingResponse';
...
const IncomingRequestDetail = (props: RequestItemDetailScreenProps) => {
  ...
  const {topResponses} = useGetTopLevelIncommingResponse(
  ...
  const msgItems = useMemo(() => {
    if (topResponses.length && responses.length) {
      let companies = topResponses
        .filter(response => response?.companyId == user.companyId)
        .map(response => ({
          target: {targetId: data.userID, targetName: data.userName},
          msgs: [response],
        }));
  ...
  const onPressMsg = (msg: any) => {
    props.navigation.navigate('RequestChat', {request: data, msg: msg, isUser: false});
  };

  const sendMessage = (msg) => {
    props.navigation.navigate('RequestChat', {
      request: data,
      msg: msg,
      isUser: false,
      newMessage,
...
export default IncomingRequestDetail;
