...
const AddDeliveryAddress = () => {
  ...
  const mutation = useAddAddressMutation();
  ...
  const getLocation = () => {
    Geolocation.getCurrentPosition(
      position => {
        ...
        setMapRegion(region);
        setLatitude(vcurrentLatitude.toString());
        setLongitude(vcurrentLongitude.toString());
        Geocoder.from(vcurrentLatitude, vcurrentLongitude)
          .then(json => {
            ...
            const streetAddressNumberItem = addressComponent.find(f =>
              f.types.includes('street_number'),
            );
            const streetNumber = streetAddressNumberItem?.long_name;
            const streetAddressItem = addressComponent.find(f =>
              f.types.includes('route'),
            );
            const streetAddressName = streetAddressItem?.long_name;
            const postalCodeItem = addressComponent.find(f =>
            ...
            setStreetAddress(`${streetAddressName} ${streetNumber}`);
  ...
  const onSaveAddress = async () => {
    ...
    if (!isValidated) {
      ...
      await mutation.mutateAsync({
        ...
        streetAddress,
        provinceID: parseInt(selectedProvince ?? '0', 10),
        cityID: selectedCity,
